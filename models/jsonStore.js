...
// Drop-in replacement: same API (readData, writeData, getNextId)
// but backed by SQLite instead of JSON files
// ======================================================
 ...
 * @param {string} filename - JSON filename (e.g., 'products.json')
 * @returns {Array} - Array of row objects
 */
function readData(filename) {
  const table = TABLE_MAP[filename];
  if (!table) {
    console.error(`[jsonStore] Unknown file: ${filename}`);
    return [];
  }
  ...
  try {
    ...
    // For orders, parse items from JSON string back to array
    if (table === 'orders') {
      return rows.map(row => ({
        ...row,
    ...
    // For products, ensure numeric types
    if (table === 'products') {
      return rows.map(row => ({
        ...row,
...
/**
 * Write entire dataset to a table (replaces all rows)
 * Used by controllers that read → modify → write back
    ...
    const transaction = db.transaction(() => {
      ...
      if (table === 'orders') {
        const stmt = db.prepare(`
          INSERT INTO orders (id, userId, userName, items, total, status, paymentStatus, paymentMethod, createdAt, updatedAt, approvedAt)
        ...
        const stmt = db.prepare(`
          INSERT INTO requests (id, userId, userName, productName, description, status, shopkeeperNote, createdAt, updatedAt)
        ...
        const stmt = db.prepare(`
          INSERT INTO products (id, name, price, description, image, stock, shopkeeperId, createdAt)
        ...
        // users / shopkeepers
        const stmt = db.prepare(`
          INSERT INTO ${table} (id, name, username, password, createdAt)
